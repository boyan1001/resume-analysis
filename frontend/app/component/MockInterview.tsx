...
export default function MockInterview() {
    ...
    const [currentStep, setCurrentStep] = useState<'input' | 'interview'>('input');
    ...
    const [interviewType, setInterviewType] = useState<'general' | 'codility' | 'behavior'>('general');
    ...
    const saveToHistory = (position: string) => {
        ...
        const newHistory = [position, ...searchHistory.filter(item => item !== position)].slice(0, 10); // 最多保留10筆
    ...
    const fetchInterviewHistory = async () => {
        try {
            ...
            const response = await fetch(`http://${window.location.hostname}:3001/interview_history`, {
                headers: {
    ...
    const startInterview = async () => {
        ...
        const fetchInitialResponse = async () => {
            try {
                ...
                const response = await fetch(`http://${window.location.hostname}:3001/interview`, {
                    method: 'POST',
                    body: formData,
                });
                const data = await response.json();
            ...
            try {
                ...
                mediaRecorderRef.current.onstop = async () => {
                    ...
                    try {
                        ...
                        setMessages(prev => prev.filter(msg => msg !== "You: (analyzing...)"));
                        if (data.user_input_text) {
                             setMessages(prev => [...prev, `You: ${data.user_input_text}`]);
                        } else {
                        ...
                        setMessages(prev => prev.filter(msg => msg !== "You: (Sending audio...)"));
    ...
    const stopRecording = () => {
        if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
            ...
            setMessages(prev => prev.filter(msg => msg !== "You: (Recording...)"));
        }
    };
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {[
                                { id: 'general', label: '一般面試', desc: '基本面試問題和職位相關討論' },
                                { id: 'codility', label: '程式 Codility', desc: '程式設計和演算法測試' },
                                { id: 'behavior', label: 'Behavior Question', desc: '行為面試和情境問題' }
                            ].map((option) => (
                                <div
                                    key={`interview-type-${option.id}`}
                                    onClick={() => setInterviewType(option.id as 'general' | 'codility' | 'behavior')}
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        padding: '12px',
                                        ...
                                        borderRadius: '8px',
                                        cursor: 'pointer',
                                        backgroundColor: interviewType === option.id ? '#f0f8ff' : '#fff',
                                        transition: 'all 0.2s',
                                    }}
                                >
                                    <div style={{
                                    ...
                                    {interviewHistory.length === 0 ? (
                                        ...
                                        interviewHistory.map((history, index) => (
                                            <div
                                                ...
                                                style={{
                                                    padding: '15px',
                                                    borderBottom: index < interviewHistory.length - 1 ? '1px solid #f0f0f0' : 'none',
                                                }}
                                            >
                                                <div style={{
                                                    fontSize: '14px',
        ...
        <div style={{
            ...
            <div style={{
                ...
                {messages.map((msg, idx) => {
                    ...
                    return (
                        <div key={`message-${idx}-${isUser ? 'user' : 'bot'}`} style={{
                            display: 'flex',
                            ...
                            <div style={{
                                ...
                                backgroundImage: isUser ? 'linear-gradient(45deg, #007bff 0%, #0056b3 100%)' : 'none',
            ...
            <div style={{
                ...
                <button
                    ...
                    style={{
                        ...
                        padding: '14px 22px',
                        borderRadius: '25px',
                        background: isRecording ? 'linear-gradient(45deg, #dc3545 0%, #c82333 100%)' : 'linear-gradient(45deg, #007bff 0%, #0056b3 100%)',
                        color: '#fff',
                        border: 'none',
                        cursor: 'pointer',
                        fontSize: '16px',
                        fontWeight: 'bold',
                        transition: 'all 0.3s ease',
                        boxShadow: isRecording ? '0 4px 10px rgba(220,53,69,0.3)' : '0 4px 10px rgba(0,123,255,0.3)',
                        outline: 'none',
